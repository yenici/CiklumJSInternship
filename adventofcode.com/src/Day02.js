...
const calculateArea = function calculateArea(dimensionsString) {
  const total = dimensionsString.split('x')
    .map(Number)
    .map((currentNumber, index, array) =>
      currentNumber * array[(index === 0 ? array.length : index) - 1]
    )
    .reduce((area, current) => ({
      wrapping: area.wrapping + (2 * current),
      slack: area.slack ? Math.min(area.slack, current) : current,
    }), { wrapping: 0, slack: undefined });
  return total.wrapping + total.slack;
};

const calculateLength = function calculateLength(dimensionsString) {
  const total = dimensionsString.split('x')
    .map(Number)
    .reduce((state, current) => (
      {
        ribbon: (state.ribbon ? state.ribbon : 0) + (2 * current),
        bow: state.bow ? state.bow * current : current,
        maxdim: state.maxdim ? Math.max(current, state.maxdim) : current,
      }
    ), { ribbon: undefined, bow: undefined, maxdim: undefined });
  return (total.ribbon - (2 * total.maxdim)) + total.bow;
};

const calculateTotalArea = function calculateTotalArea(filename) {
  // let totalArea = 0;
  // const rl = readline.createInterface({
  //   input: fs.createReadStream(filename),
  // });
  // rl.on('line', (line) => calculateArea(line))
  return fs.readFileSync(filename, 'utf8')
    .split('\n')
    .reduce((totalArea, currentPresent) => (totalArea + calculateArea(currentPresent)), 0);
};

const calculateTotalLength = function calculateTotalLength(filename) {
  return fs.readFileSync(filename, 'utf8')
    .split('\n')
    .reduce((totalLength, currentPresent) => (totalLength + calculateLength(currentPresent)), 0);
};
